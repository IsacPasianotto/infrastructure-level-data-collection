...
source .env
DATAFILE="${SCRATCH_DIR}/datatimescalesmallbis.gz"
NWORKERS=8
BATCHSIZE=1000
LIMIT=0
...
do
  ...
  do
    ...
    do
      ...
      export LOAD_OUTPUT_FILE="${RESULT_DIR}/timescalesinglepod_load_ram${r_name}_cores${c_name}_iter${i}.log"
      export OUTPUT_FILE="${RESULT_DIR}/timescalesinglepod_query_ram${r_name}_cores${c_name}_iter${i}.log"
      export HDR_FILE="${RESULT_DIR}/timescalesinglepod_latencies_ram${r_name}_cores${c_name}_iter${i}.json"
      export RES_FILE="${RESULT_DIR}/timescalesinglepod_ram${r_name}_cores${c_name}_iter${i}.json"
      echo "---------------------------------------------"
      echo " Time: $(date)"
      echo " Ram: $RAM"
      echo " Cores: $CORES"
      ...
      envsubst < 02-cluster-singlepod-tmpl.yaml > 02-cluster-singlepod.yaml
      envsubst < 03-svc-tmpl.yaml > 03-svc.yaml
      kubectl apply -f 01-sec.yaml
      kubectl apply -f 02-cluster-singlepod.yaml
      ...
      kubectl delete -f 02-cluster-singlepod.yaml
